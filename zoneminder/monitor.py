...
class Monitor:
    ...
    def is_recording(self) -> Optional[bool]:
        ...
        try:
            ...
            _LOGGER.warning("ValueError in camera %s status %s: %s", self._monitor_id, status, e)
            return False
        except TypeError as e:
            _LOGGER.warning("TypeError in camera %s status %s: %s", self._monitor_id, status, e)
            return False
        except Exception as e:
            _LOGGER.warning("Error in camera %s status %s: %s", self._monitor_id, status, e)
            return False
